...
var (
	...
	allocationMemory = prometheus.NewDesc(
		...
		[]string{"job", "group", "name", "alloc", "region", "datacenter", "node"}, nil,
	)
	...
	taskCPUTotalTicks = prometheus.NewDesc(
		...
		[]string{"job", "group", "name", "alloc", "task", "region", "datacenter", "node"}, nil,
	)
...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, a := range runningAllocs {
		...
		go func(a *api.AllocationListStub) {
			...
			ch <- prometheus.MustNewConstMetric(
				allocationCPUThrottled, prometheus.GaugeValue, float64(stats.ResourceUsage.CpuStats.ThrottledTime), alloc.Job.Name, alloc.ID, alloc.TaskGroup, alloc.Name, alloc.Job.Region, node.Datacenter, node.Name,
			)
			ch <- prometheus.MustNewConstMetric(
